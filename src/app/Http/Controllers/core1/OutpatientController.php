        ...
        */
        $registrations = $appointmentsRaw->map(function ($apt) use ($user) {
            ...
            */
            ...
            // Admin & Head Nurse cannot click
            if (in_array($user->role, ['admin', 'head_nurse'])) {
                $canAction = false;
            }

            /*
            |--------------------------------------------------------------------------
            | Priority Label for Nurse
            |--------------------------------------------------------------------------
            */
            $priority = '';
            ...
            if (
                $user->role === 'nurse' &&
                $apt->patient->assigned_nurse_id == $user->id
            ) {
                $priority = ' (PRIORITY)';
            }
...
$followUps = \App\Models\core1\MedicalRecord::where('record_type', 'follow_up')
    ...
    ->when($user->role === 'nurse', function($q) use ($user) {
        // Only follow-ups for patients assigned to this nurse
        $q->whereHas('patient', fn($q2) => $q2->where('assigned_nurse_id', $user->id));
    })
    ->get()
    ->map(function($record){
    ...
    ->when($user->role === 'nurse', fn($q) => $q->where('assigned_nurse_id', $user->id))
    ->get();




        return view('core1.outpatient.index', compact(
            'stats',
            'appointments',
            'registrations',
            'prescriptions',
    ...
    public function createPrescription()
{
    ...
    // Only doctor or nurse can create prescriptions
    if (!in_array($user->role, ['doctor', 'nurse'])) {
        abort(403);
    }
    ...
    $patients = $user->role === 'doctor' 
        ...
        : collect(); // nurse can have custom logic if needed
    ...
    return view('core1.outpatient.create_prescription', compact(
...
public function storeFollowUp(Request $request)
{
    ...
    // Only doctor or nurse can schedule follow-ups
    if (!in_array($user->role, ['doctor','nurse'])) {
        abort(403);
    }
...
public function updateFollowUp(Request $request, $id)
{
    ...
    // Only doctor or nurse can modify follow-ups
    if (!in_array($user->role, ['doctor','nurse'])) {
        abort(403);
    }
